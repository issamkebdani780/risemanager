...
const Demo = () => {
    ...
    const [orders, setOrders] = useState([
        { id: '#2843', name: 'Sami K.', status: 'Confirmé', color: 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400', amount: '12,500 DA' },
        { id: '#2842', name: 'Amine Dj.', status: 'En Attente', color: 'bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400', amount: '8,200 DA' },
        { id: '#2841', name: 'Lydia M.', status: 'En Livraison', color: 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400', amount: '15,000 DA' },
        { id: '#2840', name: 'Bachir V.', status: 'Retour', color: 'bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400', amount: '5,400 DA' }
    ]);
    ...
    const addOrder = () => {
        ...
        setOrders([{ id: newId, name: 'Nouveau Client', status: 'En Attente', color: 'bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400', amount: '0 DA' }, ...orders]);
    };
                    ...
                    <div className="space-y-6 animate-fadeIn">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {[
                                ...
                                { label: 'Commandes', val: orders.length.toString(), color: 'text-slate-900 dark:text-white' },
                        ...
                        <div className="p-6 bg-white dark:bg-slate-800 rounded-[24px] border border-slate-100 dark:border-slate-700 shadow-sm h-56 relative overflow-hidden group/chart transition-colors">
                            <div className="flex justify-between items-center mb-8">
                                <div className="text-[11px] font-extrabold text-[#1e293b] dark:text-white uppercase tracking-tight">Performance Hebdomadaire</div>
                                    ...
                                    {filteredOrders.length > 0 ? filteredOrders.map((row, i) => (
                                        <tr key={row.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 transition-colors group">
                                            ...
                                            <td className="p-4 text-[10px]">
                                                <span className={`px-2 py-0.5 rounded-md font-bold ${row.color} dark:bg-opacity-20`}>{row.status}</span>
                                            </td>
                                        ...
                                        <tr><td colSpan="4" className="p-10 text-center text-slate-400 dark:text-slate-600 italic">Aucun résultat trouvé...</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div className="space-y-4">
                            {filteredClients.length > 0 ? filteredClients.map((client) => (
                                <div key={client.id} className="p-4 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm flex items-center justify-between group transition-colors">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className={`text-[9px] font-bold uppercase tracking-widest ${client.color} bg-white dark:bg-slate-900 shadow-sm px-2 py-0.5 rounded-lg border border-slate-50 dark:border-slate-700`}>{client.score}</div>
                                ...
                                <div className="p-10 text-center text-slate-400 dark:text-slate-600 italic">Aucun client trouvé...</div>
                            )}
                        </div>
                    </div>
                );
                ...
                return (
                    <div className="p-8 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm space-y-8 animate-fadeIn text-center transition-colors">
                        <div className="space-y-2">
                            <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Projection de profit</div>
                ...
                return (
                    <div className="grid grid-cols-2 gap-4 animate-fadeIn">
                        <div className="p-5 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm col-span-2 md:col-span-1 transition-colors">
                            <div className="text-xs font-bold text-slate-400 dark:text-slate-500 mb-6 uppercase tracking-widest">Sources de Ventes</div>
                            <div className="space-y-4">
                                {[
                        ...
                        <div className="p-5 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm col-span-2 md:col-span-1 flex flex-col items-center justify-center space-y-4 transition-colors">
                            <div className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">ROI Publicitaire</div>
                            <div className="text-4xl font-black text-primary">x4.2</div>
                            <div className="text-[10px] text-emerald-500 font-bold bg-emerald-50 dark:bg-emerald-900/40 px-3 py-1 rounded-full">Optimal</div>
                        </div>
                    </div>
                );
            case 'Blacklist':
                return (
                    <div className="p-6 bg-rose-50 dark:bg-rose-900/10 rounded-2xl border border-rose-100 dark:border-rose-900/20 shadow-sm animate-fadeIn transition-colors">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                <div className="text-sm font-bold text-rose-900 dark:text-rose-100 uppercase tracking-tight">Vigilance Fraude</div>
                                <div className="text-xs text-rose-600 dark:text-rose-400 font-medium">Détection automatique des mauvais payeurs.</div>
                            </div>
                        </div>
                ...
                <div className="max-w-4xl mx-auto text-center mb-20 lg:mb-32 space-y-6">
                    <div className="flex justify-center animate-slide-up">
                        <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-50 dark:bg-slate-900 rounded-full border border-slate-100 dark:border-slate-800 shadow-sm">
                            ...
                            <span className="text-[10px] font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest leading-none">Visite Guidée</span>
                        </div>
                    </div>
