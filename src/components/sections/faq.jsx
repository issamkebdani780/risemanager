...
const FAQ = () => {
    ...
    return (
        <section className="py-24 lg:py-40 bg-white overflow-hidden" id="faq">
            <div className="container mx-auto px-4 sm:px-6">
                ...
                <div className="max-w-3xl mx-auto text-center mb-20 space-y-6">
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-full border border-slate-100">
                        ...
                        <span className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest leading-none">Questions Fréquentes</span>
                    </div>
                ...
                <div className="max-w-3xl mx-auto space-y-4">
                    {questions.map((faq, i) => (
                        <div 
                            key={i} 
                            className={`group border rounded-[32px] transition-all duration-500 animate-slide-up ${openIndex === i ? 'bg-[#fbfcfd] border-primary/20 shadow-xl shadow-slate-200/50' : 'bg-white border-slate-100'}`}
                        ...
                        >
                            ...
                            >
                                <span className={`text-base lg:text-lg font-bold transition-colors ${openIndex === i ? 'text-heading' : 'text-slate-600 group-hover:text-heading'}`}>
                                    {faq.q}
                                </span>
                                <div className={`w-10 h-10 rounded-2xl border flex items-center justify-center transition-all duration-500 ${openIndex === i ? 'bg-primary border-primary text-white rotate-180' : 'border-slate-100 text-slate-400 bg-white group-hover:border-slate-200'}`}>
